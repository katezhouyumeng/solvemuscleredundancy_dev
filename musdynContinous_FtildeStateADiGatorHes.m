...
cadainput3_11 = input.auxdata.shift;
%User Line: cadainput3_11 = input.auxdata.shift;
[cadaoutput3_1,cadaoutput3_2] = ADiGator_ForceEquilibrium_FtildeState(cadainput3_1,cadainput3_2,cadainput3_3,cadainput3_4,cadainput3_5,cadainput3_6,cadainput3_7,cadainput3_8,cadainput3_9,cadainput3_10,cadainput3_11);
...
for cadaforcount1 = 1:5
    ...
    cada1tempf1 =  zeros(1,43);
    cada2tempf1 = zeros(1,43);
    ...
    cada1tf1 = sparse(Gator1Data.Index12,Gator1Data.Index13,cada2f1,43,43);
    cada2f1 = cada1f2dV*cada1tf1;
    cada1f4dV = full(cada2f1);
...
dadt.dV = zeros(cada2f1,86);
dadt.dVdV = zeros(size(dadt.dV,1),172);
for cadaforcount2 = 1:43
    m.f = cadaforvar2.f(:,cadaforcount2);
    %User Line: m = cadaforvar2(:,cadaforcount2);
    cada2f1 = size(e.f,1);
    cada1td1 = zeros(cada2f1,43);
    ...
    cada2f4 = zeros(2,1);
    cada2f4(1:length(cada2tempf1)) = cada2tempf1;
    cada2td1 = zeros(size(cadaoutput2_1.dV,1),172);
...
cada2f1 = size(dadt.f,1);
cada1td1 = zeros(cada2f1,129);
cada1td1dV = dadt.dVdV;
cada1td1(:,Gator1Data.Index18) = dadt.dV;
cada1td1(:,Gator1Data.Index19) = dFtilde.dV;
...
cada1f1 = e.f.^2;
cada1tf2 =  ones(43,1);
cada2f1 = cada1tf2(Gator1Data.Index20);
cada1tf1 = sparse(Gator1Data.Index21,Gator1Data.Index22,cada2f1,43,43);
cada2tf1 = sparse(Gator2Data.Index4,Gator2Data.Index5,cada1tf1(Gator2Data.Index3),43,43);
cada2f1dV = full(cada1f1dVdV*cada2tf1);
cada2f1 = cada1f1dV*cada1tf1;
cada1f2dVdV = cada2f1dV;
cada1f2dV = full(cada2f1);
...
cada1tf2 =  ones(5,1);
cada1tf1 =  zeros(5,5);
cada2f1 = cada1tf2(Gator1Data.Index23);
cada1tf1(Gator1Data.Index24) = cada2f1;
cada2tf1 = zeros(5,5);
cada2tf1(Gator2Data.Index7) = cada1tf1(Gator2Data.Index6);
...
cada1f7dV = dFtilde.dV/10;
cada1f7 = dFtilde.f/10;
cada2f1dV = 1.*cada1f7.^(1-1).*cada1f7dV;
cada2f1 = cada1f7.^1;
cada2f2dV = 2.*cada2f1dV;
cada2f2 = 2*cada2f1;
...
cada1f8 = cada1f7.^2;
cada1tf2 =  ones(43,1);
cada2f1 = cada1tf2(Gator1Data.Index27);
cada1tf1 = sparse(Gator1Data.Index28,Gator1Data.Index29,cada2f1,43,43);
cada2tf1 = sparse(Gator2Data.Index12,Gator2Data.Index13,cada1tf1(Gator2Data.Index11),43,43);
cada2f1dV = full(cada1f8dVdV*cada2tf1);
cada2f1 = cada1f8dV*cada1tf1;
cada1f9dVdV = cada2f1dV;
cada1f9dV = full(cada2f1);
cada1f9 = sum(cada1f8,2);
cada1f10dVdV = w2.f.*cada1f9dVdV;
cada1f10dV = w2.f*cada1f9dV;
cada1f10 = w2.f*cada1f9;
cada2f1 = size(cada1f6dV,1);
cada1td1 = zeros(cada2f1,91);
...
cada2td1 = zeros(size(cada1td1,1),91);
cada2td1(:,Gator2Data.Index14) = cada2f2dV;
...
%User Line: phaseout.integrand = sum(e.^2,2)+ w1.*sum(aT.^2,2)+ w2*sum((dFtilde/10).^2,2);
phaseout.dynamics.dV_size = [86 178];
phaseout.dynamics.dV_location = Gator1Data.Index32;
phaseout.integrand.dV_size = 178;
phaseout.integrand.dV_location = Gator1Data.Index33;
phaseout.path.dV_size = [48 178];
phaseout.path.dV_location = Gator1Data.Index34;
phaseout.dynamics.dVdV_size = [phaseout.dynamics.dV_size,178];
phaseout.dynamics.dVdV_location = [phaseout.dynamics.dV_location(Gator2Data.Index17,:), Gator2Data.Index18];
phaseout.integrand.dVdV_size = [phaseout.integrand.dV_size,178];
phaseout.integrand.dVdV_location = [phaseout.integrand.dV_location(Gator2Data.Index19,:), Gator2Data.Index20];
phaseout.path.dVdV_size = [phaseout.path.dV_size,178];
phaseout.path.dVdV_location = [phaseout.path.dV_location(Gator2Data.Index21,:), Gator2Data.Index22];
end
...
cada1td1 = zeros(cada2f1,86);
cada1td1dV = zeros(size(cada1td1,1),86);
...
cada2td1 = zeros(size(cada1tf1dV,1),3784);
...
cada2td1 = zeros(size(cada1td1dV,1),3784);
...
cada2td1 = zeros(size(cada1td1dV,1),7396);
...
function [err,FT] = ADiGator_ForceEquilibrium_FtildeState(a,fse,dfse,lMT,vMT,params,Fvparam,Fpparam,Faparam,Atendon,shift)
global ADiGator_musdynContinous_FtildeStateADiGatorHes
...
cada1f2 = ones(cada1f1,1);
shift = cada1f2*shift;
%User Line: shift = ones(size(a,1),1)*shift;
%User Line: % Inverse tendon force-length characteristic
cada1f1dV = fse.dV;
cada1f1 = fse.f + 0.25;
cada1f2dV = cada1f1dV;
cada1f2 = cada1f1 - shift;
cada1f3dV = 5*cada1f2dV;
cada1f3 = 5*cada1f2;
cada2f1dV = -1./cada1f3.^2.*cada1f3dV;
cada2f1 = 1./cada1f3;
cada1f4dVdV = cada1f3dV.*cada2f1dV;
cada1f4dV = cada2f1.*cada1f3dV;
cada1f4 = log(cada1f3);
cada1f5dVdV = cada1f4dVdV./Atendon;
cada1f5dV = cada1f4dV./Atendon;
cada1f5 = cada1f4./Atendon;
lTtilde.dVdV = cada1f5dVdV; lTtilde.dV = cada1f5dV;
lTtilde.f = cada1f5 + 0.995;
%User Line: lTtilde = log(5*(fse + 0.25 - shift))./Atendon + 0.995;
...
cada1f2 = 0.2*Atendon;
cada1f3dVdV = lTtilde.dVdV; cada1f3dV = lTtilde.dV;
cada1f3 = lTtilde.f - 0.995;
cada1f4dVdV = Atendon.*cada1f3dVdV;
cada1f4dV = Atendon.*cada1f3dV;
cada1f4 = Atendon.*cada1f3;
cada2f1dV = exp(cada1f4).*cada1f4dV;
cada2f1 = exp(cada1f4);
cada2td1 = cada1f4dV.*cada2f1dV;
cada2td1 = cada2td1 + cada2f1.*cada1f4dVdV;
...
cada1f5dV = cada2f1.*cada1f4dV;
cada1f5 = exp(cada1f4);
cada1f6dVdV = cada1f2.*cada1f5dVdV;
cada1f6dV = cada1f2.*cada1f5dV;
cada1f6 = cada1f2.*cada1f5;
cada2f1 = size(cada1f1dV,1);
cada1td1 = zeros(cada2f1,86);
cada2f1dV = -cada1f1dV./cada1f6.^2.*cada1f6dV;
cada2f1 = cada1f1dV./cada1f6;
cada1td1dV = cada2f1dV;
cada1td1(:,Gator1Data.Index1) = cada2f1;
cada2f1 = cada1td1(:,Gator1Data.Index2);
cada2f2dV = -cada1f1dV;
cada2f2 = uminus(cada1f1);
cada2f3dV = 2.*cada1f6.^(2-1).*cada1f6dV;
cada2f3 = cada1f6.^2;
cada2td1 = zeros(size(cada2f2dV,1),86);
...
cada2tf1 = cada1f6dV(:,Gator2Data.Index4);
cada2td1 = cada2tf1.*cada2f4dV;
cada2td1(:,Gator2Data.Index5) = cada2td1(:,Gator2Data.Index5) + cada2f4.*cada1f6dVdV;
cada2f5dV = cada2td1;
cada2f5 = cada2f4.*cada1f6dV;
cada2f6dV = cada2f5dV;
cada2f6 = cada2f1 + cada2f5;
cada2td1 = zeros(size(cada1td1,1),129);
...
vT.f = cada1f1./cada1f6;
%User Line: vT = lTs.*dfse./(0.2*Atendon.*exp(Atendon.*(lTtilde-0.995)));
...
cada2td1 = zeros(size(cada1tf1dV,1),129);
...
cada1td1dV = cada1f2dVdV; cada1td1 = cada1f2dV;
cada2td1 = zeros(size(cada1td1dV,1),172);
...
cada2td1 = zeros(size(cada1tf1dV,1),215);
...
cada2td1 = zeros(size(cada1tf1dV,1),258);
...
cada2td1 = zeros(size(cada2f1dV,1),258);
cada2td1(:,Gator2Data.Index52) = cada2f1dV;
cada2td1 = cada2td1 + cada2f2dV;
cada2f3dV = cada2td1;
cada2f3 = cada2f1 + cada2f2;
cada2td1 = zeros(size(cada1td1,1),344);
...
cada2td1 = zeros(size(cada1tf1dV,1),344);
